...
combined = pd.concat([may_df, june_df, july_df, august_df])
...
fig = px.bar(
    ...
    category_orders={"Month": ["May", "June", "July", "August"]}
)
...
def demographic_chart(df, filter_column, filter_value, color_map, title, months):
    ...
    totals = (
        group.groupby("Month")
        .size()
        .reindex(["May", "June", "July", "August"], fill_value=0)
        .reset_index(name="Total Voters")
    )
...
with st.expander("Policy Area Support/Oppose Breakdown by Party (All Months, 2×2 Grid)", expanded=False):
    vote_map = {
        "Hate It": 1,
        "Dislike It": 2,
        "Not Sure": 3,
...
# Combine
df = pd.concat([may_df, june_df, july_df, august_df])
...
# Month toggle
month = st.radio("Select Month", ["May", "June", "July", "August"], horizontal=True)
